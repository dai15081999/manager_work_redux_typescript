    ...
    <div className="modal is-active">
      ...
      <div className="modal-card">
        <header className="modal-card-head has-text-centered">
          <p className="modal-card-title">Bạn có muốn xóa không ?</p>
        </header>
        <div className="modal-card-body">
          <h2 className="is-size-5 has-text-centered">
        Tất cả các nhiệm vụ liên quan đến danh sách này sẽ bị xóa</h2>
          <div className="content">
            {list?.tasks.length === 0 ?
              <p className="has-text-centered pt-4 mb-0">Danh sách trống!</p>
              :
              <ul>
                {list?.tasks.map(task => (
        ...
        <footer className="modal-card-foot">
          <button type="button" className="button is-danger" onClick={deleteListHandler}>Xóa</button>
          <button type="button" className="button" onClick={hideModalHandler}>Hủy</button>
        </footer>
      </div>
    </div>
  );
