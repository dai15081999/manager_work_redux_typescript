...
const Tasks: FC<TasksProps> = ({ tasks }) => {
  ...
  const tasksTable = (
    <table className="table is-striped is-fullwidth">
      <thead>
        <tr>
          <th>Task</th>
          <th className="has-text-centered">Sửa</th>
          <th className="has-text-centered">Xóa</th>
        </tr>
      </thead>
      <tbody>
        {
  ...
  return(
    <section className="section">
      <h2 className="is-size-4 has-text-centered">Danh sách đã chọn</h2>
      {tasks.length === 0 ? <p className="py-4 has-text-centered">Không có nv nào</p> : tasksTable}
    </section>
  );
}
